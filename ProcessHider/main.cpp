...
// ═══ CHANGED: Shared-memory monitor globals ═══
HANDLE   g_hMapFile = NULL;
LPVOID   g_pMapView = NULL;
DWORD*   g_pHideActive = nullptr;
bool     g_hideActive = true;   // default = keep hiding until told to stop
// ══════════════════════════════════════════════
    ...
    PULONG ReturnLength)
{
    ...
    // CHANGED: If hiding is disabled, return original data unchanged
    if (!g_hideActive) return status;
    ...
    PULONG ReturnLength)
{
    ...
    // CHANGED: If hiding is disabled, don't modify
    if (!g_hideActive) return status;
...
DWORD_PTR WINAPI Hooked_SHGetFileInfoW(
    ...
    UINT uFlags)
{
    // CHANGED: If hiding is disabled, pass straight through
    if (!g_hideActive) {
        return g_OrigSHGetFileInfoW(pszPath, dwFileAttributes, psfi, cbFileInfo, uFlags);
    }
...
// ============================================================================
// Shared memory  (CHANGED: keeps mapping open for live polling)
// ============================================================================
void ReadSharedMemory() {
    if (g_pMapView) return; // Already connected

    g_hMapFile = OpenFileMappingA(FILE_MAP_ALL_ACCESS, FALSE, "GetProcessName");
    if (!g_hMapFile) {
        DllLog("OpenFileMappingA failed: %lu", GetLastError());
        return;
    }

    g_pMapView = MapViewOfFile(g_hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, 256);
    if (!g_pMapView) {
    ...
    // CHANGED: Connect to hide-active flag at offset 140
    g_pHideActive = (DWORD*)(data + 140);
    if (g_pHideActive) {
        g_hideActive = (*g_pHideActive != 0);
    }

    DllLog("Config loaded: target=%ls injectorPID=%lu hideActive=%d",
        g_targetProcessName ? g_targetProcessName : L"(none)", g_injectorPid,
        g_hideActive ? 1 : 0);
}

// CHANGED: Monitor thread polls shared memory flag every 500 ms
DWORD WINAPI MonitorHideFlagThread(LPVOID) {
    while (true) {
        if (g_pHideActive) {
            bool newState = (*g_pHideActive != 0);
            if (newState != g_hideActive) {
                g_hideActive = newState;
                DllLog("Hide state changed in PID:%lu -> %s",
                    GetCurrentProcessId(), g_hideActive ? "HIDING" : "SHOWING");
            }
        } else {
            // Try to connect if not yet available
            ReadSharedMemory();
        }
        Sleep(500);
    }
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH:
        ...
        PatchTargetPEB();
        CloseHandle(CreateThread(NULL, 0, ResolveTargetPidThread, NULL, 0, NULL));
        // CHANGED: Start monitor thread to watch for "stop hiding" signal
        CloseHandle(CreateThread(NULL, 0, MonitorHideFlagThread, NULL, 0, NULL));
        break;
    ...
    case DLL_PROCESS_DETACH:
        ...
        // CHANGED: Clean up persistent shared memory handles
        if (g_pMapView) {
