...
DWORD WINAPI KillSwitchThread(LPVOID lpParam) {
    if (g_expiryMinutes == 0) {
        Log("[KILL SWITCH] No expiry limit set. Running indefinitely.");
        return 0;
    }
...
bool setup_shared_memory(const string& targetName) {
    ...
    *targetPidPtr = targetPid;
    ...
    // ═══ NEW: Write expiry minutes to shared memory offset 136 ═══
...
// ═══ ENTRY POINT ═══
// ═══ NEW: Accept expiry minutes from Python via command-line argument ═══
int main(int argc, char* argv[]) {
    ...
    cout << "[CONSOLE] ShadowHide Injector started. PID: " << g_myPid << endl;
    // ═════════════════════


    // ═══ NEW: Parse expiry minutes passed by Python launcher ═══
    if (argc > 1) {
    ...
    g_targetProcess = targetName;
    Log("[MAIN] Target: " + targetName + " -> svchost.exe | PID: " + to_string(g_myPid));
    // ═══ NEW: Log received expiry time ═══
    Log("[MAIN] Expiry Minutes received: " + to_string(g_expiryMinutes));
    ...
    // ═══ NEW: Start kill-switch timer thread ═══
