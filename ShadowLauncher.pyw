...
# ═══ NEW: Trusted network time check ═══
...
def _validate_system_time(max_drift=300):
    ...
    if trusted is None:
        # Cannot reach time servers — allow but we can't verify
        return True, None
    local = int(time.time())
    drift = local - trusted  # positive = local ahead, negative = local behind
...
def _verify(key):
    ...
    # ═══ NEW: Validate local system time before using it ═══
...
# ═══ NEW: Offline error message helper ═══
...
# ═══ NEW: Dual validation (offline + online) ═══
def _validate_license(key, hwid):
    ...
    # If offline has a critical error (not just expired), return immediately
    ...
    # If online has network error, we can't compare — return it
    if online.get("code") == "E_NET":
        return online

    # If online has specific auth errors (revoked, device locked)
    ...
    # MISMATCH — one says active, the other says expired/failed
...
def _launch_injector(expiry_minutes=None):
    ...
    if expiry_minutes is not None:
        params = str(expiry_minutes)

    ret = ctypes.windll.shell32.ShellExecuteW(
        None, "runas", injector_path, params,
        os.path.dirname(injector_path), 1
    )
    if ret > 32:
        return True
